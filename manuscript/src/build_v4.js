...
const C = require("./content_v4.js");

const FIG = process.env.HCHO_FIG || require("path").resolve(__dirname, "../../output/figures") + "/";
...
children.push(new Paragraph({ children: [new PageBreak()] }));
children.push(H1("Supplementary figures (move to the Supplement)"));
C.supplement.forEach(f => children.push(...figure(f.file, f.w, f.h, f.caption)));
...
Packer.toBuffer(doc).then(buf => {
  const out = require("path").resolve(__dirname, "../COATTS_TEMPO_HCHO_AMT_draft_v4.docx");
